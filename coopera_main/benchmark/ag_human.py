...
# CUDA_VISIBLE_DEVICES="0,3" python coopera_main/benchmark/ag_human.py --collab-type 2 --collab-setting 1 --start-logic-robot True --start-logic-lora True
...
if __name__ == "__main__":
    ...
    eval_dir = pathlib.Path(results_path) / "benchmark" / f"collaboration_{collab_type}" / f"setting_{collab_setting}" / "ag_human"
    ...
    for day_idx, scene_idx, human_idx in zip(config['days'], config['scenes'], config['humans']):
        ...
        eval_dir_per_human = pathlib.Path(results_path) / "benchmark" / f"collaboration_{collab_type}" / f"setting_{collab_setting}" / "ag_human" / str(human_idx).zfill(5) / scene_id
        eval_csv_data = []
        ...
        for j, time_ in enumerate(times):
            ...
            robot_retrieved_intentions = []
            robot_retrieved_predicates = []

            robot_conversation_hist = intention_discovery_mllm(results_path, human_idx, scene_id, [day, j, time_], [os.path.join(video_dir, "robot_scene_camera_rgb_video"), os.path.join(video_dir, "human_third_rgb_video")], [robot_retrieved_intentions, robot_retrieved_predicates, human_thoughts[0]], [inferred_profile[human_idx], inferred_traits[human_idx]], temperature_dict, model_dict, method="ag_human", collab=collab_type, setting=collab_setting, gpt=use_gpt_robot, start_over=start_logic_robot)
            ...
            _, data_test = create_data_none(pred_intention_sentence_list, [None]*intentions_num, time_, [inferred_profile[human_idx], inferred_traits[human_idx]], [robot_retrieved_intentions, robot_retrieved_predicates], data_type="intention", collab=collab_type)
            ...
            for k, (selected_intention_sentence, selected_sampled_static_obj_dict) in enumerate(zip(selected_intention_sentence_list, selected_sampled_static_obj_dict_list)):
                ...
                _, data_test_batch = create_data_none([None, robot_thoughts_batch, extract_inhand_obj_robot(robot_acts_batch, collab=collab_type)], [None]*predicates_num, time_, [inferred_profile[human_idx], inferred_traits[human_idx]], [robot_retrieved_intentions, robot_retrieved_predicates], data_type="predicates", collab=collab_type)
            ...
            intentions_approval_res = intention_approval_mllm(results_path, human_idx, scene_id, [day, j, time_], [gt_intention_sentence, pred_intention_sentence_list], temperature_dict, model_dict, method="ag_human", collab=collab_type, setting=collab_setting, start_over=start_logic_robot)[0][1]
            ...
            for k, _ in enumerate(selected_intention_sentence_list):
                predicates_approval_res = predicate_approval_mllm(results_path, human_idx, scene_id, [day, file_idx+k, time_], [human_thoughts, gt_intention_sentence], extract_inhand_obj_human(human_acts, collab=collab_type), robot_thoughts[k*predicates_num:(k+1)*predicates_num], extract_inhand_obj_robot(robot_acts[k*predicates_num:(k+1)*predicates_num], collab=collab_type), temperature_dict, model_dict, method="ag_human", collab=collab_type, setting=collab_setting, start_over=start_logic_robot)[0][1]
            ...
            category_approval = []
            for k, _ in enumerate(selected_intention_sentence_list):
                category_approval_res = category_approval_mllm(results_path, human_idx, scene_id, [day, file_idx+k, time_], human_thoughts, extract_inhand_obj_human(human_acts, collab=collab_type), robot_thoughts[k*predicates_num:(k+1)*predicates_num], extract_inhand_obj_robot(robot_acts[k*predicates_num:(k+1)*predicates_num], collab=collab_type), temperature_dict, model_dict, method="ag_human", collab=collab_type, setting=collab_setting, start_over=start_logic_robot)[0][1]
            ...
            # No intentions and tasks history after the discussion
            robot_intentions_hist[human_idx].append("none")
            robot_retrieved_predicates_gt = []

            for k, (robot_thought, human_thought) in enumerate(zip(robot_thoughts, human_thoughts)):
                robot_predicates_hist[human_idx].append("none")
            ...
            data_train_intentions_batch_1, _ = create_data_none(pred_intention_sentence_list, labels, time_, [inferred_profile[human_idx], inferred_traits[human_idx]], [robot_retrieved_intentions, robot_retrieved_predicates_gt], data_type="intention", collab=collab_type)
            data_train_intentions_batch_2, _ = create_data_none([gt_intention_sentence], ["Yes"], time_, [inferred_profile[human_idx], inferred_traits[human_idx]], [robot_retrieved_intentions, robot_retrieved_predicates_gt], data_type="intention", collab=collab_type)
            ...
            data_train_predicate_batch, _ = create_data_none([None, train_thoughts, train_acts], labels, time_, [inferred_profile[human_idx], inferred_traits[human_idx]], [robot_retrieved_intentions, robot_retrieved_predicates_gt], data_type="predicates", collab=collab_type)
            data_train_predicates.extend(data_train_predicate_batch)

            lora_intention_dir = pathlib.Path(results_path) / "lora_models" / f"collaboration_{collab_type}" / f"setting_{collab_setting}" / "ag_human" / "intention" / str(human_idx).zfill(5) / scene_id / day
            lora_predicates_dir = pathlib.Path(results_path) / "lora_models" / f"collaboration_{collab_type}" / f"setting_{collab_setting}" / "ag_human" / "predicates" / str(human_idx).zfill(5) / scene_id / day
            ...
            for k in range(len(predicates_approval)):
                append_evaluation_row(
                    ...
                    category_approval, eval_predicates_semantic_within_day, method="ag_human"
                )
        ...
        # Robot Does Not Infer Human Traits
        eval_big_five_across_days_latest, eval_big_five_across_days_voting = "none", "none"
        
        # Data Visualization
        save_evaluation_results(eval_csv_path, eval_txt_path, eval_csv_data, data_train_intentions, data_train_predicates, str(day_counter), method="ag_human")
