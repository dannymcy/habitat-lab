...
from habitat.gpt.query import *
from habitat.gpt.prompts.robot.prompt_intention_inference import *
from habitat.gpt.prompts.robot.prompt_predicates_discovery import *
...
def parse_args():
    ...
    parser.add_argument('--start-logic-robot', type=lambda x: x.lower() == 'true',
                        default=True,
                        help='Restart robot intention/task discovery and finetuning (True/False, default: True)')
...
# CUDA_VISIBLE_DEVICES="3" python coopera_main/benchmark/finetuning.py --collab-type 2 --collab-setting 1 --start-logic-robot True
...
if __name__ == "__main__":
    ...
    # GPT or Llama for robot (always GPT for this baseline)
    use_gpt_robot = True
    ...
    # List of available GPT models for finetuning (https://platform.openai.com/docs/guides/model-optimization)
    model_dict = {
        ...
        "motion_description": "gpt-5.1",
        "finetuning": "gpt-4.1-2025-04-14",
        "finetuning_base_model": "gpt-4.1-2025-04-14"
    }
    ...
    eval_dir = pathlib.Path(results_path) / "benchmark" / f"collaboration_{collab_type}" / f"setting_{collab_setting}" / "finetuning"
    ...
    for day_idx, scene_idx, human_idx in zip(config['days'], config['scenes'], config['humans']):   
        ...
        eval_dir_per_human = pathlib.Path(results_path) / "benchmark" / f"collaboration_{collab_type}" / f"setting_{collab_setting}" / "finetuning" / str(human_idx).zfill(5) / scene_id
        eval_csv_data = []
        eval_txt_path = eval_dir_per_human / f"train_data_cls_human_{human_idx}_day_{day_idx}.txt"
        os.makedirs(eval_dir_per_human, exist_ok=True)

        # suffix string should have at most 64 characters
        gpt_finetune_suffix = str(pathlib.Path(f"co_{collab_type}") / f"set_{collab_setting}" / str(human_idx).zfill(5) / scene_id / day)
        gpt_finetune_data_path = eval_dir_per_human / f"train_data_gpt_human_{human_idx}_day_{day_idx}.jsonl"
        gpt_finetune_model_path = eval_dir_per_human / f"finetuned_model_human_{human_idx}_day_{day_idx}.txt"
        ...
        for j, time_ in enumerate(times):
            ...
            if collab_type == 2:
                robot_conversation_hist = motion_description_mllm(results_path, human_idx, scene_id, [day, j, time_], [os.path.join(video_dir, "robot_scene_camera_rgb_video"), os.path.join(video_dir, "human_third_rgb_video")], temperature_dict, model_dict, method="finetuning", collab=collab_type, setting=collab_setting, start_over=start_logic_robot)
                motion_description = robot_conversation_hist[0][1]
            elif collab_type == 1:
                motion_description = human_thoughts[0]
            ...
            robot_conversation_hist = intention_inference_mllm(results_path, human_idx, scene_id, [day, j, time_], motion_description, [robot_retrieved_intentions, robot_retrieved_predicates], [inferred_profile[human_idx], inferred_traits[human_idx]], temperature_dict, model_dict, method="finetuning", collab=collab_type, setting=collab_setting, start_over=start_logic_robot)
            ...
            intentions_approval_res = intention_approval_mllm(results_path, human_idx, scene_id, [day, j, time_], [gt_intention_sentence, pred_intention_sentence], temperature_dict, model_dict, method="finetuning", collab=collab_type, setting=collab_setting, start_over=start_logic_robot)[0][1]
            intentions_approval, _ = extract_intention_approval(intentions_approval_res)
            
            predicates_approval_res = predicate_approval_mllm(results_path, human_idx, scene_id, [day, j, time_], [human_thoughts, gt_intention_sentence], extract_inhand_obj_human(human_acts, collab=collab_type), robot_thoughts, extract_inhand_obj_robot(robot_acts, collab=collab_type), temperature_dict, model_dict, method="finetuning", collab=collab_type, setting=collab_setting, start_over=start_logic_robot)[0][1]
            predicates_approval, _ = extract_predicate_approval(predicates_approval_res)

            category_approval_res = category_approval_mllm(results_path, human_idx, scene_id, [day, j, time_], human_thoughts, extract_inhand_obj_human(human_acts, collab=collab_type), robot_thoughts, extract_inhand_obj_robot(robot_acts, collab=collab_type), temperature_dict, model_dict, method="finetuning", collab=collab_type, setting=collab_setting, start_over=start_logic_robot)[0][1]
            ...
            # Finetune GPT with OpenAI API
            # Create training data
            data_prompt.append(infer_intention_prompt(time_, motion_description, [inferred_profile[human_idx], inferred_traits[human_idx]], [robot_retrieved_intentions, robot_retrieved_predicates_gt], collab=collab_type))
            data_response.append(infer_intention_response(time_, gt_intention_sentence))

            if collab_type == 2:
                data_prompt.append(discover_predicates_finetuning_prompt_2(time_, gt_intention_sentence, [robot_retrieved_intentions, robot_retrieved_predicates_gt, human_thoughts[0]], [inferred_profile[human_idx], inferred_traits[human_idx]], [human_sampled_static_obj_dict, dynamic_obj_room_mapping]))
                data_response.append(discover_predicates_finetuning_response_2(time_, gt_intention_sentence, human_thoughts, extract_inhand_obj_human(human_acts, collab=collab_type), predicates_num))
            elif collab_type == 1:
                data_prompt.append(discover_predicates_finetuning_prompt_1(time_, gt_intention_sentence, [robot_retrieved_intentions, robot_retrieved_predicates_gt, human_thoughts[0]], [inferred_profile[human_idx], inferred_traits[human_idx]], [human_sampled_static_obj_dict, dynamic_obj_room_mapping]))
                data_response.append(discover_predicates_finetuning_response_1(time_, gt_intention_sentence, human_thoughts, extract_inhand_obj_human(human_acts, collab=collab_type), predicates_num))
            ...
            append_evaluation_row(
                ...
                category_approval, eval_predicates_semantic_within_day, method="finetuning"
            )
            eval_csv_data.append([""] * 16)


        # Robot Inferring Human Traits
        fuzzy_traits = traits_inference_mllm(results_path, human_idx, scene_id, [day, 0, None], [robot_intentions_hist[human_idx][-13:], robot_predicates_hist[human_idx]], [inferred_profile[human_idx], inferred_traits[human_idx]], temperature_dict, model_dict, method="finetuning", collab=collab_type, setting=collab_setting, gpt=use_gpt_robot, start_over=start_logic_robot)[0][1]
        ...
        # Data Visualization
        save_evaluation_results(eval_csv_path, eval_txt_path, eval_csv_data, None, None, str(day_counter), method="finetuning")
        ...
        if day_counter < len(config['days']):
            if start_logic_robot:
                create_data_openai(data_prompt, data_response, json_path=gpt_finetune_data_path, training=True)
                finetuned_model_name = finetune_openai(gpt_finetune_data_path, gpt_finetune_suffix, gpt_finetune_model_path, n_epochs=2, model=model_dict["finetuning_base_model"])
            else:
                finetuned_model_name = load_finetuned_model_name_openai(gpt_finetune_model_path)
            model_dict["finetuning"] = finetuned_model_name