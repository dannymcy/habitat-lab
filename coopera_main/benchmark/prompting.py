...
# CUDA_VISIBLE_DEVICES="1" python coopera_main/benchmark/prompting.py --collab-type 2 --collab-setting 2 --start-logic-robot True
...
if __name__ == "__main__":
    ...
    eval_dir = pathlib.Path(results_path) / "benchmark" / f"collaboration_{collab_type}" / f"setting_{collab_setting}" / "prompting"
    ...
    for day_idx, scene_idx, human_idx in zip(config['days'], config['scenes'], config['humans']):  
        ...
        eval_dir_per_human = pathlib.Path(results_path) / "benchmark" / f"collaboration_{collab_type}" / f"setting_{collab_setting}" / "prompting" / str(human_idx).zfill(5) / scene_id
        eval_csv_data = []
        ...
        for j, time_ in enumerate(times):
            ...
            robot_conversation_hist = intention_discovery_mllm(results_path, human_idx, scene_id, [day, j, time_], [os.path.join(video_dir, "robot_scene_camera_rgb_video"), os.path.join(video_dir, "human_third_rgb_video")], [robot_retrieved_intentions, robot_retrieved_predicates, human_thoughts[0]], [inferred_profile[human_idx], inferred_traits[human_idx]], temperature_dict, model_dict, method="prompting", collab=collab_type, setting=collab_setting, gpt=use_gpt_robot, start_over=start_logic_robot)
            ...
            intentions_approval_res = intention_approval_mllm(results_path, human_idx, scene_id, [day, j, time_], [gt_intention_sentence, pred_intention_sentence], temperature_dict, model_dict, method="prompting", collab=collab_type, setting=collab_setting, start_over=start_logic_robot)[0][1]
            ...
            append_evaluation_row(
                ...
                category_approval, eval_predicates_semantic_within_day, method="prompting"
            )
            eval_csv_data.append([""] * 16)


        # Robot Inferring Human Traits
        fuzzy_traits = traits_inference_mllm(results_path, human_idx, scene_id, [day, 0, None], [robot_intentions_hist[human_idx][-13:], robot_predicates_hist[human_idx]], [inferred_profile[human_idx], inferred_traits[human_idx]], temperature_dict, model_dict, method="prompting", collab=collab_type, setting=collab_setting, gpt=use_gpt_robot, start_over=start_logic_robot)[0][1]
        ...
        # Data Visualization
        save_evaluation_results(eval_csv_path, eval_txt_path, eval_csv_data, None, None, str(day_counter), method="prompting")
